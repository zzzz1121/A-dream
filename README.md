...
当前版本已经具备电脑端脑电桥接、浏览器实时前端、M5Stack 网关监测、M5Stack 到 Microduino 的 ESP-NOW 转发、Microduino DMX 灯光控制与安全总开关。浏览器前端只显示真实串口、M5Stack 和 Microduino 回传状态；没有收到真实数据时显示 `--` / 等待，不使用假数据。继电器和步进电机的协议与状态机已预留，物理输出默认关闭，等待硬件安全确认后再启用。
...
`COM7` 需要替换为实际端口。上传、串口监视器和 Python 脚本不能同时占用同一个 COM 口。
...
前端可以查看真实实时数据，并发送系统开启、系统关闭、灯光颜色、继电器、步进电机和全部停止指令。没有收到真实 EEG / M5Stack / Microduino 状态时，界面显示 `--` 或等待；所有控制按钮都有发送中、已发送、失败反馈。若 M5Stack 串口未打开，控制请求会返回失败。
...
- Microduino 上电默认 `systemEnabled = false`，没有收到系统开启指令前所有机器默认关闭。
