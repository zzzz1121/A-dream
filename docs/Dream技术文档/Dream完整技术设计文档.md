...
当前电脑端浏览器前端已经改为真实状态控制台：没有收到真实 EEG / M5Stack / Microduino 状态时显示 `--` 或等待，不使用假数据。所有控制按钮都有发送中、已发送、失败反馈；M5Stack 串口未打开时控制指令会失败，不进入发送队列。Microduino 正式联动固件当前已启用两盏 DMX RGBW 灯和单步进驱动板台架输出；继电器物理输出仍默认关闭。
...
| 对象 | STEP | DIR | 说明 |
| --- | ---: | ---: | --- |
| 单步进驱动板 | `GPIO25` | `GPIO14` | 一个驱动板接两个电机 |

前端目标 `左`、`右`、`左右` 仍可发送目标掩码 `1 / 2 / 3`，当前 Microduino 固件会统一映射到这一个驱动板输出。
