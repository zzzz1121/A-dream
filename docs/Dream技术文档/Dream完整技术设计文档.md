...
当前电脑端浏览器前端已经改为真实状态控制台：没有收到真实 EEG / M5Stack / Microduino 状态时显示 `--` 或等待，不使用假数据。所有控制按钮都有发送中、已发送、失败反馈；M5Stack 串口未打开时控制指令会失败，不进入发送队列。Microduino 正式联动固件当前已启用两盏 DMX RGBW 灯和左右步进电机台架输出；继电器物理输出仍默认关闭。
...
M5Stack -> Microduino: ESP-NOW broadcast peer, 结构化 EEG / CMD 包
```

当前原型使用 ESP-NOW broadcast peer，便于现场快速联调。正式运行时仍建议填写 Microduino Wi-Fi MAC 并改为固定 peer，减少无关设备误收和现场干扰风险。
...
- 解析并显示 EEG 数据。
- 通过 ESP-NOW broadcast peer 转发 EEG / CMD 包给 Microduino；正式部署建议改为固定 peer。
- 显示 EEG 数据。
- 显示串口和 ESP-NOW 链路状态。
- 显示丢包和数据年龄。
- 显示 Microduino 执行状态。
...
```text
attention / meditation -> 流水亮度，取两者较大值并限制在 64-255
attention -> 色轮相位偏移
灯 1 -> 使用当前色轮相位
灯 2 -> 在灯 1 基础上增加 96/256 圈相位偏移
poorSignal > 120 -> 进入信号差提示色
EEG 超时或系统关闭 -> 灯光关闭
```
...
manualLightEnabled:
  系统开启后，使用前端 RGBW 颜色生成手动渐变流水，允许用于灯光测试
```
...
步进电机正在输出脉冲时，DMX 刷新会让出主循环时间，减少电机脉冲被灯光写帧打断的机会。
...
```text
STEPPER_LEFT_STEP_PIN
STEPPER_LEFT_DIR_PIN
STEPPER_RIGHT_STEP_PIN
STEPPER_RIGHT_DIR_PIN
STEPPER_ENABLE_PIN
LIMIT_MIN_PIN
LIMIT_MAX_PIN
```
...
| 对象 | STEP | DIR | 说明 |
| --- | ---: | ---: | --- |
| 左步进电机 | `GPIO27` | `GPIO26` | 前端目标掩码 `1` |
| 右步进电机 | `GPIO25` | `GPIO14` | 前端目标掩码 `2` |
| 左右同时 | `GPIO27/GPIO25` | `GPIO26/GPIO14` | 前端目标掩码 `3`，`0` 也按左右同时处理 |

当前 DMX 占用 `GPIO5`。正式负载接入前仍需确认驱动器电流、方向、限位、机械行程和急停。
...
- 必须有停止状态。
- 必须有最大行程或限位开关。
- 前端只发送步数和目标掩码，STEP 脉冲由 Microduino 本地生成。
...
- 使用固定发送频率 10-20 Hz。
- ESP-NOW 包带 `seq`。
- ESP-NOW 当前使用固定 channel；正式部署建议使用固定 peer MAC。
...
```text
灯光关闭或低亮度待机
步进电机输出当前已启用于台架调试，但默认保持停止或 `STEPPER_DISABLED`
继电器 OFF
等待有效 EEG 数据
等待串口和 ESP-NOW 链路稳定
```
...
- Microduino 固件版本正确。
- M5Stack 固件版本正确。
- ESP-NOW channel 一致；如改为固定 peer，M5Stack 和 Microduino MAC 配置一致。
- 串口日志正常。
- M5Stack 显示数据年龄和丢包数。
...
继电器必须在无线链路、灯光输出和安全策略稳定后再接入。步进电机当前只建议作为低风险台架调试输出使用；真实机械负载接入前必须补齐限位、行程、电流、方向和急停确认。
