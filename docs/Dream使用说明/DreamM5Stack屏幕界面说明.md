...
当前 Microduino 可根据这些值生成灯光目标色。步进电机和继电器相关逻辑也预留了脑电触发条件，但物理输出默认关闭。
...
当前固件中 Microduino 的步进电机物理输出默认关闭：

```cpp
#define DREAM_ENABLE_STEPPER_OUTPUT 0
```

因此现场大多数时候会看到 `Step:DISABLED`。正式接入电机前，需要确认引脚、驱动器、限位、电源和机械安全。
...
| 状态 | 含义 |
| --- | --- |
| `SYS:ON` | 系统已开启，允许 Microduino 执行灯光和预留机构逻辑 |
